...
int main (int argc, char *argv[])
{
   ...
   if(n%2==0)
      n = n-1;
   low_value = id * ((n-1)/2)/p;
   high_value = (id+1) * ((n-1)/2)/p;
   low_value = low_value*2 + 3;
   high_value = high_value *2 +1;
   local_low_value = 0 * ((n - 1) / 2) / p;
   local_high_value = 1 * ((n-1) / 2) / p;
   local_low_value = local_low_value *2 + 3;
   local_high_value = local_high_value *2 +1;
