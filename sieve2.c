...
int main (int argc, char *argv[])
{
   ...
   /* Add you code here  */
    ...
    for (i = 0; i < size0; i++) marked0[i] = 0;
    index0 = 0;
    prime = 3;
    do {
        if (prime * prime > low_value0)
            first = (prime * prime - low_value0)/2;
        else {
            if (!(low_value0 % prime)) first = 0;
            else if(low_value0 % prime % 2 ==0)
                first = prime - ((low_value0 % prime)/2);
            else
                first = (prime - (low_value0 % prime))/2;
        }
